...
export default new class AnimeTosho {
  ...
  async single({ anidbEid, resolution, exclusions }, options) {
    if (!navigator.onLine) return [];
    if (!anidbEid) throw new Error("AnimeTosho: No anidbEid provided");
    const query = this._buildQuery({ resolution, exclusions });
    const res = await fetch(`${this.url}?eid=${anidbEid}${query}`);
  ...
  async batch({ anidbAid, resolution, exclusions, episode }, options) {
    if (!navigator.onLine) return [];
    if (!anidbAid) throw new Error("AnimeTosho: No anidbAid provided");
    const query = this._buildQuery({ resolution, exclusions });
    const res = await fetch(`${this.url}?order=size-d&aid=${anidbAid}${query}`);
    if (!res.ok) throw new Error(`AnimeTosho: HTTP ${res.status}`);
    const data = (await res.json()).filter(
  ...
  async test() {
    try {
      const res = await fetch(this.url);
      if (!res.ok) throw new Error(`AnimeTosho: HTTP ${res.status} — is the site down?`);
      return true;
    } catch (error) {
      throw new Error(`AnimeTosho: Could not reach feed. ${error.message}`);
    }
  }
};
